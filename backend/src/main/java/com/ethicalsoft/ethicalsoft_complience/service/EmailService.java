...
public class EmailService {
	...
	public void sendRecoveryEmail( String to, String code ) {
		try {
			log.info("[email] Enviando código de recuperação para {}", to);
			...
			log.info("[email] Código de recuperação enviado para {}", to);
		} catch ( MessagingException | IOException | TemplateException e ) {
			log.error("[email] Falha ao enviar email de recuperação para {}", to, e);
			throw new EmailSendingException( "Failed to send recovery email to " + to, e );
		}
	}

	@Async
	public void sendNewUserCredentialsEmail( String to, String firstName, String tempPassword, String projectName, String adminName ) {
		try {
			log.info("[email] Enviando credenciais temporárias para {}", to);
			var message = mailSender.createMimeMessage();
			var helper = new MimeMessageHelper(message, true);
			...
			mailSender.send( message );
			log.info("[email] Credenciais temporárias enviadas para {}", to);
		} catch ( MessagingException | IOException | TemplateException e ) {
			log.error("[email] Falha ao enviar credenciais temporárias para {}", to, e );
			throw new EmailSendingException( "Falha ao enviar credenciais para " + to, e );
		}
	}

	@Async
	public void sendProjectAssignmentEmail( String to,
	                                       String firstName,
	                                       String projectName,
	                                       Long projectId,
	                                       String adminName,
		...
		try {
			log.info("[email] Enviando notificação de vinculação ao projeto {} para {}", projectName, to);
			var message = mailSender.createMimeMessage();
			var helper = new MimeMessageHelper( message, true );

			Map<String, Object> model = new HashMap<>();
			var activeContext = buildProjectQuestionnaireContext(projectId);
			model.put( "firstName", firstName );
			model.put( "projectName", projectName );
			model.put( "projectLink", buildFrontendLink( "/projects/" + projectId ) );
			model.put( "adminName", adminName );
			model.put( "adminEmail", adminEmail );
			model.put( "roles", roleNames );
			model.put( "timelineSummary", timelineSummary );
			model.put( "startDateFormatted", formatDate( startDate ) );
			model.put( "deadlineFormatted", formatDate( deadline ) );
			model.put( "nextQuestionnaireFormatted", formatDate( nextQuestionnaireDate ) );
			model.put( "supportEmail", supportEmail );
			model.put( "environment", environment );
			...
			mailSender.send( message );
			log.info("[email] Notificação de projeto enviada para {}", to);
		} catch ( MessagingException | IOException | TemplateException e ) {
			log.error("[email] Falha ao enviar notificação de projeto para {}", to, e );
			throw new EmailSendingException( "Falha ao enviar notificação de projeto para " + to, e );
		}
	}

	public void sendQuestionnaireReminderEmail(String to, QuestionnaireReminderContext context) {
		try {
			log.info("[email] Enviando lembrete de questionário {} para {}", context.questionnaireId(), to);
			var message = mailSender.createMimeMessage();
			var helper = new MimeMessageHelper(message, true);
			...
			log.error("[email] Falha ao enviar lembrete de questionário {} para {}", context.questionnaireId(), to, e);
