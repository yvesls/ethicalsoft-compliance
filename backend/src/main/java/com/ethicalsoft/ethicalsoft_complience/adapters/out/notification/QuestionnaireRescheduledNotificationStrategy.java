...
@Slf4j
public class QuestionnaireRescheduledNotificationStrategy implements NotificationTypeStrategy {
    ...
    public NotificationType type() {
        return NotificationType.QUESTIONNAIRE_RESCHEDULED;
    }
    ...
    public void send(SendNotificationCommand command) {
        ...
        String oldStartDate = str(command, "oldStartDate");
        String oldEndDate = str(command, "oldEndDate");
        String newStartDate = str(command, "newStartDate");
        String newEndDate = str(command, "newEndDate");
        String deadlineWarning = str(command, "deadlineWarning");
        ...
        if (recipients.isEmpty()) {
            log.warn("[notification-reschedule] Nenhum destinatário encontrado. projeto={}", projectId);
            return;
        }
        ...
        recipients.forEach(to -> {
            Map<String, String> placeholders = Map.ofEntries(
                    ...
                    Map.entry("oldStartDate", oldStartDate),
                    Map.entry("oldEndDate", oldEndDate),
                    Map.entry("newStartDate", newStartDate),
                    Map.entry("newEndDate", newEndDate),
                    Map.entry("deadlineWarning", deadlineWarning),
        ...
        log.info("[notification-reschedule] Notificação de reagendamento enviada. questionário={} projeto={} destinatários={}",
