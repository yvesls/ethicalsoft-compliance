...
export class NotificationService {
	...
	private formatErrorMessage(error: unknown): string {
		...
		if (this.isApiError(error)) {
			...
			return `**${this.translate.instant('notification.error')} ${status}** - ${error.errorType ?? 'ERROR'}: ${message || getErrorMessage(status)}`
		}
	...
	private showModal(
		...
		const confirmLabel = this.translate.instant('common.confirm')
      ...
      <div class="small-card modal-content" @modalAnimation>
        ...
        <img class="modal-icon" src="assets/icons/${type}.svg" alt="${title}">
