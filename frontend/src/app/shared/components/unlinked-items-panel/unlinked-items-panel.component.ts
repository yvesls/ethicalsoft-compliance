...
export class UnlinkedItemsPanelComponent {
  ...
  get hasItems(): boolean {
    return this.unlinkedRoles.length > 0 || this.unlinkedStages.length > 0
      || this.orphanedRoles.length > 0 || this.orphanedStages.length > 0;
  }

  get totalCount(): number {
    return this.unlinkedRoles.length + this.unlinkedStages.length
      + this.orphanedRoles.length + this.orphanedStages.length;
  }
