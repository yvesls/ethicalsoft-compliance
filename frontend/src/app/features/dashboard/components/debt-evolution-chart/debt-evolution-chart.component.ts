...
export class DebtEvolutionChartComponent implements OnChanges {
	...
	private buildChart(): void {
		const validEntries = this.entries.filter((e) => e.ethicsDebtPercent !== null || e.techDebtPercent !== null)
		...
		this.chartOptions = {
			...
			legend: {
				bottom: 8,
				data: [
					this.translate.instant('dashboard.chart.debt_ethics'),
					this.translate.instant('dashboard.chart.debt_tech'),
				],
			},
			xAxis: {
				type: 'category',
