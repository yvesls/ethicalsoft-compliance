...
import { ConsolidatedAnswerDTO, ConsolidatedAnswerFilters, Page } from '../../interfaces/dashboard.interface'
...
export class ConsolidatedAnswersPageComponent implements OnInit {
	...
	private loadRoles(): void {
		this.roleService.getRoles().subscribe({
			next: (roles) => {
				this.roleOptions.set(roles.map((r) => ({ value: r.id.toString(), label: r.name })))
			},
		})
	}
	...
	load(): void {
		...
		const request$ =
			this.mode === 'questionnaire'
				...
				: this.dashboardService.getProjectConsolidatedResponses(this.projectId, filters)
