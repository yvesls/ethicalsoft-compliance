...
export class EditCascataProjectFormComponent implements OnInit {
	...
	private buildStageFormGroup(stage: ProjectStageDetail): FormGroup {
		...
		const dateRange =
			startStr && endStr ? `${FormUtils.formatDateBR(startStr)} - ${FormUtils.formatDateBR(endStr)}` : ''
	...
	private getRoleNamesFromIds(roleIds: number[] | undefined): string[] {
		...
		return roleIds.map((id) => this.roleNameById.get(Number(id))).filter((name): name is string => Boolean(name))
	}
	...
	private saveFormStateToCache(): void {
		try {
			...
			sessionStorage.setItem(EditCascataProjectFormComponent.FORM_STATE_CACHE_KEY, JSON.stringify(state))
		} catch (e) {
				...
				next: (response) => {
					try {
						...
						const totalChanges =
							...
							summary.stagesRemoved +
							summary.questionnairesAdded +
							summary.questionnairesUpdated +
							summary.questionnairesRemoved +
							summary.representativesAdded +
							summary.representativesUpdated +
							summary.representativesRemoved +
							summary.questionsAdded +
							summary.questionsUpdated +
							summary.questionsRemoved

						if (summary.blockedReasons?.length) {
							const reasons = summary.blockedReasons.join('\n• ')
							this.notificationService.showError(
								this.translate.instant('notifications.project_form.update_blocked', { reasons })
							)
							return
						}
						...
						if (summary.warnings?.length) {
							...
							this.notificationService.showWarning(
								this.translate.instant('notifications.project_form.updated_with_warnings', { warns })
							)
						} else {
						...
						this.notificationService.showError(
							this.translate.instant('notifications.project_form.save_error')
						)
					}
				},
				error: (err) => {
		...
		if (this.isConflictError(err)) {
			...
			try {
				...
				if (parsed.errors?.length) {
					...
					this.notificationService.showError(
						this.translate.instant('notifications.project_form.operation_blocked', { errorList })
					)
					return
				}
			} catch (parseError) {
	...
	private ensureStageDurationDays(stages: ProjectStageDetail[], projectStartDate: string): ProjectStageDetail[] {
		...
		for (const stage of stages) {
			...
			const estimated = this.estimateDurationDays(
				stageStart,
				stage.applicationStartDate,
				stage.applicationEndDate
			)
		...
		for (let pos = 0; pos < sorted.length; pos++) {
			...
			if (existing) {
				...
				const fallbackQuestions =
					lastQuestionsTemplate.length > 0 ? JSON.parse(JSON.stringify(lastQuestionsTemplate)) : []
				this.questionnairesFormArray.push(
					this.fb.group({
						_entityId: [null],
						name: [stepName, [Validators.required]],
