...
export class StageCascataModalComponent implements OnInit {
	...
	private sequenceUniquenessValidator(): ValidatorFn {
		return (control: AbstractControl): ValidationErrors | null => {
			...
			return this.existingSequences.includes(sequence) ? { duplicateSequence: true } : null
		}
	}
	...
	private calculateApplicationRange(): void {
		...
		const stagesForSimulation =
			this.actionType === ActionType.EDIT && this.currentStageIndex !== undefined
				? contextualStages.map((stage) => (stage.index === this.currentStageIndex ? { ...candidate } : stage))
