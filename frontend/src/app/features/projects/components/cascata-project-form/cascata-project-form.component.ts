...
export class CascataProjectFormComponent extends BasePageComponent<CascataProjectRouteParams> implements OnInit {
		...
		this.projectForm = this.fb.group(
			{
				...
				startDate: [null, [Validators.required, ProjectDatesValidators.startDateAllowsExistingStages()]],
				deadline: [
					null,
					[
						Validators.required,
	...
	protected override loadParams(params: RouteParams<CascataProjectRouteParams>): void {
	...
	private getExistingStagesDataWithIndex(): {
		index: number
		weight: number
		durationDays: number
		sequence: number
	...
	private addNewStage(newStage: StageCascataData): void {
		...
		this.stepsFormArray.push(
			this.fb.group({
				...
				dateRange: [
					`${FormUtils.formatDateBR(newStage.applicationStartDate)} - ${FormUtils.formatDateBR(newStage.applicationEndDate)}`,
				],
	...
	private loadPanelData(panelKey: PanelKey): void {
		...
		switch (panelKey) {
			case 'steps': {
				if (this.selectedTemplateData.stages?.length && !this.hasUserModifiedSteps()) {
		...
		switch (panelKey) {
			case 'project': {
				...
				const hasRelevantGroupError =
					groupErrors?.['dateRange'] || groupErrors?.['deadlineTooEarly'] || groupErrors?.['startDateTooLate']
			...
			case 'steps': {
				const stepsArray = this.projectForm.get('steps') as FormArray
				return Boolean(
					stepsArray && stepsArray.valid && stepsArray.length > 0 && !this.hasStagesExceedDeadlineError()
				)
			}
	...
	private createProjectWithTemplate(): void {
		...
		try {
			...
			const message =
				error instanceof Error ? error.message : this.translate.instant('projects.messages.prepare_error')
	...
	saveDraft(): void {
		...
		try {
			...
			const message =
				error instanceof Error ? error.message : this.translate.instant('projects.messages.prepare_draft_error')
			this.notificationService.showError(message)
			return
		}
	...
	private mapStageTemplateToForm(stage: TemplateStageDTO, fallbackSequence = 1): CascataStageFormValue {
		...
		const sequence = Number.isFinite(parsedSequence) && parsedSequence > 0 ? parsedSequence : fallbackSequence
