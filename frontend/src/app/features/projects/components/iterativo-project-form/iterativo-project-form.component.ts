import { Component, ChangeDetectionStrategy, inject, DestroyRef, ChangeDetectorRef, OnInit } from '@angular/core'
...
export class IterativoProjectFormComponent extends BasePageComponent<IterativoProjectRouteParams> implements OnInit {
  ...
  protected override onInit(): void {
    ...
    this.projectForm = this.fb.group(
      ...
      {
        validators: [CustomValidators.dateRange('startDate', 'deadline')],
      }
    )
  ...
  private setupIterationCountCalculation(): void {
    ...
    startDateControl?.valueChanges.pipe(takeUntilDestroyed(this.destroyRef)).subscribe(calculateIterationCount)

    deadlineControl?.valueChanges.pipe(takeUntilDestroyed(this.destroyRef)).subscribe(calculateIterationCount)
  ...
  private setupIterationListener(): void {
    ...
    startDateControl?.valueChanges.pipe(takeUntilDestroyed(this.destroyRef)).subscribe(regenerateIterations)

    deadlineControl?.valueChanges.pipe(takeUntilDestroyed(this.destroyRef)).subscribe(regenerateIterations)

    iterationDurationControl?.valueChanges.pipe(takeUntilDestroyed(this.destroyRef)).subscribe(regenerateIterations)
  }
  ...
  private mapRepresentativeRoleIds(rep: Representative): number[] {
    if (Array.isArray(rep.roles) && rep.roles.length) {
      return rep.roles.map((role) => Number(role.id)).filter((id) => Number.isFinite(id) && id > 0)
    }
  ...
  private getRoleNamesFromIds(roleIds: number[] | undefined): string[] {
    ...
    return roleIds.map((id) => this.roleNameById.get(Number(id))).filter(Boolean) as string[]
  }
  ...
  private loadTemplateData(templateId: string): void {
    this.templateStore
      ...
      .subscribe({
        next: (template) => {
          if (template.iterations && Array.isArray(template.iterations)) {
            template.iterations.sort((a, b) => {
  ...
  private buildIterationsForm(data?: Iteration[]): FormArray {
    ...
    return this.fb.array(
      iterationsData.map((iteration, index) =>
        this.fb.group({
          name: [
            iteration.name || this.translate.instant('projects.form.iteration_fallback', { n: index + 1 }),
            [Validators.required],
          ],
          ...
          dateRange: [
            `${FormUtils.formatDateBR(iteration.applicationStartDate)} - ${FormUtils.formatDateBR(iteration.applicationEndDate)}`,
          ],
        })
      )
    )
  ...
  addRepresentative(): void {
    ...
    const createdSubscription = modalRef.representativeCreated.subscribe(
      (newRepresentative: RepresentativeData) => {
  ...
  editRepresentative(index: number): void {
    ...
    const updatedSubscription = modalRef.representativeUpdated.subscribe(
      (updatedRepresentative: RepresentativeData) => {
  ...
  editQuestionnaire(index: number): void {
    ...
    if (!questionnaire) {
      this.notificationService.showWarning(
        this.translate.instant('notifications.project_form.load_questionnaire_error')
      )
      return
    }
  ...
  private createProjectWithTemplate(): void {
    ...
    try {
      ...
      LoggerService.error('IterativoProjectForm: Erro ao construir payload de criação.', error)
      const message =
        error instanceof Error
          ? error.message
          : this.translate.instant('projects.form.validation.prepare_data_error')
    ...
    this.projectStore
      ...
      .pipe(
        switchMap((project) => {
          if (!project?.id) {
            throw new Error(this.translate.instant('projects.form.validation.project_id_not_found'))
          }
      ...
      .subscribe({
        next: () => {
          this.notificationService.showSuccess(
            this.translate.instant('projects.messages.created_with_template')
          )
  ...
  saveDraft(): void {
    ...
    try {
      ...
      LoggerService.error('IterativoProjectForm: Erro ao construir payload de rascunho.', error)
      const message =
        error instanceof Error
          ? error.message
          : this.translate.instant('projects.form.validation.prepare_draft_error')
      this.notificationService.showError(message)
      return
    }
    ...
    const draftKey = this.draftCacheService.projectDraftKey('iterativo')
    this.draftCacheService.save(draftKey, formValue, 'project-creation', { type: 'iterativo' })
  ...
  private buildDraftPayload(): ProjectCreationPayload {
    const formValue = this.projectForm.getRawValue() as IterativoProjectFormValue
    const projectName = (formValue.name || '').trim()
    const iterationDuration = Number(formValue.iterationDuration) || 0
  ...
  private buildIterationPayload(): IterationPayload[] {
    ...
    return this.iterationsFormArray.controls
      ...
      .filter((iteration) =>
        Boolean(
          iteration.applicationStartDate && iteration.applicationEndDate && Number.isFinite(iteration.weight)
        )
      )
  }
  ...
  private isPanelValid(panelKey: PanelKey): boolean {
    switch (panelKey) {
      case 'project': {
        ...
        const allRequiredValid = requiredControls.every(
          (controlName) => this.projectForm.get(controlName)?.valid
        )
  ...
  onAttemptedToggle(panelKey: PanelKey): void {
    this.notificationService.showWarning(
      this.translate.instant('projects.form.validation.complete_previous_panels', { panel: panelKey })
    )
  }
  ...
  private loadPanelData(panelKey: PanelKey): void {
    ...
    switch (panelKey) {
      case 'stages': {
        ...
        if (this.selectedTemplateData.stages?.length && !this.hasUserModifiedStages()) {
      ...
      case 'representatives': {
        if (this.selectedTemplateData.representatives?.length) {
          ...
          if (repsArray.length === 0) {
            const repsData = this.selectedTemplateData.representatives.map(
              (rep: TemplateRepresentativeDTO) => this.mapTemplateRepresentativeToForm(rep)
            )
  ...
  private generateQuestionnaires(): void {
    ...
    const cacheByIteration = new Map((previousValues || []).map((item) => [item.iteration || item.name, item]))
  ...
  private extractRoleIds(roles?: RoleSummary[], fallbackNames?: string[]): number[] {
    const idsFromRoles = Array.isArray(roles)
      ? roles.map((role) => Number(role.id)).filter((id) => Number.isFinite(id) && id > 0)
      : []
  ...
  private extractRoleNames(roles?: RoleSummary[], fallbackNames?: string[]): string[] {
    if (roles?.length) {
      return roles.map((role) => role.name?.trim() ?? '').filter((name) => name.length > 0)
    }
