...
export class AiTokenSettingsComponent implements OnInit {
  ...
  save(): void {
    ...
    this.aiTokenService.updateToken(token).subscribe({
      ...
      error: (err) => {
        const msg = err?.error?.translatedMessage ?? err?.error?.message ?? this.translate.instant('ai.token.errors.not_configured');
  ...
  remove(): void {
    this.notificationService.showConfirm(
      this.translate.instant('ai.token.form.remove'),
      () => {
